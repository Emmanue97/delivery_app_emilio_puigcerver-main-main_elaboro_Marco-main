...
import 'expense_tile.dart'; // Asegúrate de ajustar la ruta según tu estructura de proyecto
        ...
        body: Column(
          children: [
            ...
            Padding(
              ...
              child: Column(
                children: [
                  // Botones de Añadir Método de Pago, Gasto, Miembro y Ver Balances de Gastos...
                  const SizedBox(height: 10),
                  SizedBox(
                    width: double.infinity,
                    child: ElevatedButton(
                    ...
                    child: ElevatedButton(
                      onPressed: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => PremiumFeatureScreen(
                                groupId: widget.groupId), //Funciones Premium
                            // ExpenseBalancePage(groupId: widget.groupId), //Para evitar ver los anuncios descomenta esta linea y comenta la anterior
                          ),
                        );
                      },
                  ...
                  StreamBuilder<QuerySnapshot>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      final sortedExpenses = expenses.toList()
                        ..sort((a, b) {
                          final dateA =
                              (a.data() as Map<String, dynamic>)['fecha_compra']
                                  as Timestamp;
                          final dateB =
                              (b.data() as Map<String, dynamic>)['fecha_compra']
                                  as Timestamp;
                          return dateB.compareTo(dateA); // Orden descendente
                        });
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return ExpenseTile(
                              expense: expense,
                              expenseRef: sortedExpenses[index].reference);
                        },
                      );
                    },
                  ),
                            ...
                            child: ListTile(
                              ...
                              subtitle: Column(
                                ...
                                children: [
                                  const SizedBox(height: 8),
                                  Text(
                                    'ID Miembro: ${member['id_miembro'] ?? 'N/A'}',
                                    style: const TextStyle(color: Colors.grey),
                                  ),
                                  const SizedBox(height: 8),
                                  Text(
                                    'Salario: \$${member['salario']?.toStringAsFixed(2) ?? 'N/A'}',
                                    style: const TextStyle(color: Colors.grey),
                                  ),
                                ],
                              ),
                                ...
                                onPressed: () async {
                                  final confirm = await showDialog<bool>(
                                    ...
                                    builder: (context) => AlertDialog(
                                      title:
                                          const Text('Confirmar eliminación'),
                                      content: const Text(
                                          '¿Deseas quitar a este miembro del grupo?'),
                                      // Parte 3 de GroupDetailsPage
                                  ...
                                  if (confirm == true) {
                                    await FirebaseFirestore.instance
                                        ...
                                        .update({
                                      'id_grupo': FieldValue.arrayRemove([
                                        widget.groupId
                                      ]), // Quitar el ID del grupo
                                    });
                                  }
                                },
                              ),
